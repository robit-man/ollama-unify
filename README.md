...
`--gpu` creates an exclusive whole-GPU reservation. While that reservation is pending, active, or revoking, broker-owned Ollama lanes can continue on other selected GPUs. The wrapper gives the child exactly the reserved UUIDs through `CUDA_VISIBLE_DEVICES`, heartbeats its lease, and releases it after the foreground command stops. The declared MiB value validates that the selected devices have enough live free memory; it is not an enforceable fractional VRAM carve-out. Without an explicit GPU scope, the broker retains the conservative host-wide drain for compatibility because it cannot prove where the external process will allocate.
...
3. `docker gpu ready <token>` — marks the external allocation active. Scoped leases already keep unreserved GPUs open; legacy unscoped leases reopen Ollama here.
...
The broker looks up the installed model size and capabilities through `/api/tags`, adds configurable model and VRAM margins, checks each selected GPU independently against live free memory, reserves host headroom, and admits the request only if every missing lane fits. Each accepted lane is a broker-owned `ollama serve` process pinned to one selected GPU and a private loopback port. The broker loads the requested model through its native completion, embedding, or reranking endpoint and verifies residency before it marks the lane ready. The response exposes only safe lane IDs/GPU assignments and the public API; clients continue sending all inference to port `11434` and cannot select or bypass a private lane. If a client skips the capacity call, its first ordinary inference request lazily starts one fitting lane and concurrent requests expand the pool only while additional selected GPUs still fit. Model tags that omit `:latest` share the same lane identity; separately named aliases remain distinct.

Inference uses one broker-owned FIFO and one capacity reconciler. It can replace the least-recently-used idle lane when a different model needs a full pool, without disturbing active lanes. Disconnected waiters are removed before they can create phantom inference. Permanent admission failures are returned immediately and removed from the FIFO, so one incompatible or missing model cannot starve later requests. Runtime status and the well-known endpoint report queue depth, age, per-model demand, phases, and aggregate admission/cancellation/failure timing. Responses return `X-Ollama-Unify-Lane`, `X-Ollama-Unify-Request-Id`, exact admission-only `X-Ollama-Unify-Queue-Ms`, and `X-Ollama-Unify-Queue-Position` headers. Idle lanes are unloaded and stopped automatically, including embedding-only models that reject the generation endpoint. Stable new or increased foreign CUDA allocations affect only broker lanes on selected GPUs; process departure and unselected-GPU churn do not destroy the pool. A scoped cooperative lease temporarily drains the pool while its reservation is committed, then permits new capacity only on unreserved GPUs throughout the external transition. A legacy unscoped lease keeps the host-wide drain. Broker shutdown also terminates its complete child process groups, and lanes are intentionally ephemeral across restart.

Clients that provide `X-Ollama-Unify-Admission-Wait-Ms` get a bounded admission cycle. If a lease transition or full pool lasts longer than that value, the broker returns `503`, `Retry-After: 2`, `X-Ollama-Unify-Retryable: true`, `retryable: true`, `reason_code: "gpu_admission_unavailable"`, the request ID, and the current queue summary instead of leaving the HTTP connection silent for the full drain timeout. The request remains safe to retry and retains FIFO/cancellation accounting. This header limits only broker admission; it does not limit model loading or token generation after admission.
...
| Host remainder | Effective RAM minus the hard cap. This is an informational outside-cgroup budget; startup compares `MemAvailable` with the service `MemoryMax` budget |
