...
The broker looks up the installed model size through `/api/tags`, adds configurable model and VRAM margins, checks each selected GPU independently against live free memory, reserves host headroom, and admits the request only if every missing lane fits. Each accepted lane is a broker-owned `ollama serve` process pinned to one selected GPU and a private loopback port. The broker loads the requested model with zero generated tokens and verifies residency before it marks the lane ready. The response exposes only safe lane IDs/GPU assignments and the public API; clients continue sending all inference to port `11434` and cannot select or bypass a private lane. If a client skips the capacity call, its first ordinary inference request lazily starts one fitting lane and concurrent requests expand the pool only while additional selected GPUs still fit. Model tags that omit `:latest` share the same lane identity; separately named aliases remain distinct.

Inference uses one broker-owned FIFO and one capacity reconciler. It can replace the least-recently-used idle lane when a different model needs a full pool, without disturbing active lanes. Disconnected waiters are removed before they can create phantom inference. Runtime status and the well-known endpoint report queue depth, age, per-model demand, phases, and aggregate admission/cancellation timing. Responses return `X-Ollama-Unify-Lane`, `X-Ollama-Unify-Request-Id`, exact admission-only `X-Ollama-Unify-Queue-Ms`, and `X-Ollama-Unify-Queue-Position` headers. Idle lanes are unloaded and stopped automatically. Stable new or increased foreign CUDA allocations affect only broker lanes on selected GPUs; process departure and unselected-GPU churn do not destroy the pool. A cooperative external lease drains active streams and stops every broker-owned lane before it is granted; after `ready`, new capacity is fitted from the VRAM that remains. Broker shutdown also terminates its complete child process groups, and lanes are intentionally ephemeral across restart.
