...
render_safety_preflight_script() {
  cat <<'PREFLIGHT'
#!/bin/sh
# Generated by ollama-unify. Refuse to start Ollama when MemAvailable cannot
# cover the service's complete MemoryMax budget or the host is stalled in
# reclaim. On modern systemd this runs as an ExecCondition, so a refusal skips
# startup without marking the unit failed.
set -eu
...
if [ "$mem_available_mib" -lt "$required_mib" ]; then
  echo "ollama-unify preflight: refusing start; ${mem_available_mib} MiB available, ${required_mib} MiB required for MemoryMax" >&2
  exit 75
fi
...
render_safety_service_directives() {
  ...
  if [ "$SYSTEMD_VERSION" -ge 243 ]; then
    printf '%s\n' "ExecCondition=${SAFETY_PREFLIGHT_PATH} ${SAFETY_HOST_MEMORY_MAX_MIB} ${SAFETY_MEMORY_PRESSURE_LIMIT_PERCENT}"
  else
    printf '%s\n' "ExecStartPre=${SAFETY_PREFLIGHT_PATH} ${SAFETY_HOST_MEMORY_MAX_MIB} ${SAFETY_MEMORY_PRESSURE_LIMIT_PERCENT}"
  fi
